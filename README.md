...
- 小米运动自动刷步数，小米运动APP现已改名 `Zepp Life/Zepp`，为方便说明，后面还是称其为小米运动。但下载注册时请搜索 `Zepp Life/Zepp`，两个APP都可以使用，功能相同。
- 在Zepp上注册账号后建议**先绑定虚拟手环、绑定微信**，再使用“提交步数”测试自己的账号刷步数是否正常：
    - https://task.ytool.cc/home/login.php
- 必须先绑定虚拟手环，如果在没有绑定的情况下，先提交步数，会导致该账号被拉黑，无法和微信进行同步，即使后续绑定了手环，也无法同步！
